...
try:
    with pyodbc.connect(CONNECTION_STRING) as conn:
        ...
        write_log(f"Executando Robbyson Matriz D45 (SPID: {spid})...")
        cursor.execute(f"EXEC exec sp_ins_Robbyson_Matriz_D45")
        write_log(f"Procedure Robbyson Matriz D45 finalizada...")
        conn.commit()
