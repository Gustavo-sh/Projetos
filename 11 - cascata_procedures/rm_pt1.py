...
try:
    with pyodbc.connect(CONNECTION_STRING) as conn:
        ...
        write_log(f"Executando Robbyson Matriz parte 1 (SPID: {spid})...")
        cursor.execute(f"EXEC exec sp_ins_Robbyson_Matriz_pt1")
        write_log(f"Procedure Robbyson Matriz parte 1 finalizada...")
        conn.commit()
