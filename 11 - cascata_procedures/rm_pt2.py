...
try:
    with pyodbc.connect(CONNECTION_STRING) as conn:
        ...
        write_log(f"Executando Robbyson Matriz parte 2 (SPID: {spid})...")
        cursor.execute(f"EXEC exec sp_ins_Robbyson_Matriz_pt2")
        write_log(f"Procedure Robbyson Matriz parte 2 finalizada...")
        conn.commit()
