...
IF OBJECT_ID('tempdb..#base_sistema_matriz') IS NOT NULL DROP TABLE #base_sistema_matriz;
IF OBJECT_ID('tempdb..#base_sistema_matriz_filtrada') IS NOT NULL DROP TABLE #base_sistema_matriz_filtrada;
IF OBJECT_ID('tempdb..#metas_m3') IS NOT NULL DROP TABLE #metas_m3;
IF OBJECT_ID('tempdb..#horas_formatadas') IS NOT NULL DROP TABLE #horas_formatadas;
...
select distinct atributo, LTRIM(RTRIM(LEFT(id_nome_indicador, CHARINDEX('-', id_nome_indicador) - 1))) as id_indicador, meta 
into #base_sistema_matriz
from sistema_matriz (nolock) 
where periodo >= DATEADD(d, 1, eomonth(GETDATE(), -3))
and periodo < DATEADD(d, 1, eomonth(GETDATE(), 0))

select atributo, id_indicador, meta
into #base_sistema_matriz_filtrada
from #base_sistema_matriz
WHERE
    id_indicador not in (6,34,15,48,49)
    and id_indicador not in (select id_indicador from rby.indicador (nolock) where formula_meta not like '%fatorCalc%' and formula_meta not like '%meta%' and indicador_nome <> 'Descontinuado')
    AND id_indicador not in (select distinct id_indicador from rby.indicador (nolock) where indicador_nome like '%pausa%')

select
    bf.atributo, 
    bf.id_indicador, 
    case 
        when id_formato = 4 then cast(DATEDIFF(SECOND,'00:00:00',TRY_CAST(bf.meta AS TIME)) as decimal(10,2))
        else cast(bf.meta as decimal(10,2))
    end as meta
into #horas_formatadas
from #base_sistema_matriz_filtrada bf
left join rby_indicador (nolock) i on bf.id_indicador = i.id_indicador

select
    hf.atributo, 
    hf.id_indicador, 
    avg(meta) as meta_media_tres_meses
into #metas_m3
from #horas_formatadas hf
left join rby_indicador (nolock) i on hf.id_indicador = i.id_indicador
group by hf.atributo, hf.id_indicador
...
SELECT
    ...
    faixa_inferior,
    faixa_superior,
    m3.meta_media_tres_meses,
    case
        when b.id_indicador in (select id_indicador from rby.indicador (nolock) where (indicador_nome like '%tmo%' or indicador_nome like '%tma%' or indicador_nome like '%tmt%'))
            then 0
        else 1
    end as pode_facilitar
FROM #base b
...
left join #metas_m3 m3
    on b.id_indicador = m3.id_indicador
    and a.atributo = m3.atributo
...
drop table #agg
drop table #base
drop table #base_sistema_matriz
drop table #base_sistema_matriz_filtrada
drop table #metas_m3
drop table #horas_formatadas
    """
...
def metodo_variacao(df, meta):
    ...
    meta_media_tres_meses = None
    #meta_media_tres_meses = float(df.meta_media_tres_meses.iloc[0]) if not pd.isna(df.meta_media_tres_meses.iloc[0]) else None
    pode_facilitar = int(df.pode_facilitar.iloc[0]) if not pd.isna(df.pode_facilitar.iloc[0]) else None
    nova_meta = None

    if tipo == "Maior Melhor":
        if atingimento >= 1:
            if meta_media_tres_meses is not None and not pd.isna(meta_media_tres_meses):
                nova_meta = meta_media_tres_meses + (meta_media_tres_meses * 0.03)
            else:
...
def encontrar_meta(df):
    ...
    pode_facilitar = df.pode_facilitar.iloc[0]
    ...
    if p_original < faixa_inf and pode_facilitar == 0:
        return meta_original, p_original, "Original/NF", meta_original
